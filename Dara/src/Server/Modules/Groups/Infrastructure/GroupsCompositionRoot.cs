...
public class GroupsCompositionRoot : ModuleCompositionRootBase
{

    protected override void ConfigureReferences(ModuleReferencesConfiguration.ModuleReferencesConfigurationBuilder builder)
    {
        builder
            .WithApplicationAssembly(IGroupsModule.ContainingAssembly)
            .WithInfrastructureAssembly(GroupsModule.ContainingAssembly)
            .WithDeclaredModuleInterface(IGroupsModule.AsTypeKey)
    ...
    {
        AddStandardDataAccess<GroupsContext>(builder);
    }
    ...
    {
        AddStandardMessaging<GroupsContext>(builder);
    }
}